...
def clean_footer_junk(text):
    blacklist = [
        "More recipes", "More in recipes and meal ideas", "More in Start for Life",
        "Start for Life", "Last reviewed", "Next review due"
    ]
...
def parse_metadata_from_text(text):
    ...
    metadata = {
        "title": lines[0].replace("recipe", "").strip().title() if lines else None,
        "description": lines[1].strip() if len(lines) > 1 else None,
        "age_range": None, "prep_time": None, "cook_time": None, "portions": None,
        "ingredients": [], "weaning_tip": None
    }
    ...
    for line in lines:
        ...
        if "age:" in stripped.lower():
            ...
            metadata["portions"] = int(match.group()) if match else None
        elif stripped.lower() == "ingredients":
            section = "ingredients"
        elif stripped.lower() in {"method", "instructions"}:
            section = None
        elif "weaning tip" in stripped.lower():
            metadata["weaning_tip"] = stripped.split(":", 1)[1].strip() if ":" in stripped else None
...
def process_url(url, key, max_words=150, overlap=30):
    ...
    try:
        clean_text = clean_footer_junk(fetch_clean_text(url))
    ...
    output = [{
        "chunk": chunk, "chunk_id": f"{slug}_{i}", "title": metadata["title"],
        "description": metadata["description"], "source_url": url,
        "scraped_at": datetime.utcnow().isoformat(), "tokens": estimate_tokens(chunk),
        "age_range": metadata["age_range"], "prep_time": metadata["prep_time"],
        "cook_time": metadata["cook_time"], "portions": metadata["portions"],
        "ingredients": metadata["ingredients"], "weaning_tip": metadata["weaning_tip"],
        "meal_type": "unknown"
...
if __name__ == "__main__":
    ...
    for key, url in recipe_urls.items():
        process_url(url, key)
